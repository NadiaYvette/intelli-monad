    ...
    Right (A.Object o) ->
      case KM.lookup "tools" o of
        Just (A.Array xs) -> do
          ...
          L.sort names `shouldBe` ["call_bash_script", "organ_check_boundary", "organ_diagnostics", "organ_find_symbol", "organ_ingest", "organ_plan_stub", "organ_repo_map", "search_arxiv"]
          nSchema `shouldBe` 8
